...
################################
# Spinodal decomposition smoothed SAV
################################
method = "SAV"
n_relax = 4
for GridSize in [128]
    ...
    open("./Job_specs.csv", "a", lock=false) do f
        writedlm(f, [Dates.format(date_time, "mm/dd/yyyy HH:MM:SS") "spinodal_decomp_smoothed" "Julia" "NMG" GridSize epsilon dt 1e-5 max_it 1e4 pathname elapsed_time mem_allocated / 1e6], ",")
    end
