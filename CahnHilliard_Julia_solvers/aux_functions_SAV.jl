...
function r0_fun(phi0, hx, hy, C0)
    fphi = f(phi0)
    r0 = sqrt(hx * hy * sum(fphi) + C0)
    return r0
end

function extback(x_ext)
...
function df(phi, gamma0)
    return phi .^ 3 .- (1 .+ gamma0) .* phi
end

function Lap_SAV(phi, k2)
...
function b_fun(phi, hx, hy, C0, gamma0)
    E1 = fft_filtered(f(phi))
    return df(phi, gamma0) ./ sqrt.(E1[1, 1] * hx * hy .+ C0)
end

function g_fun_CN(phi0, r0, b, dt, hx, hy, epsilon2, gamma0, Beta, C0, k2)
