...
import os
from .aux_functions import calculate_mass, calculate_discrete_energy
...
def CahnHilliard_NMG(phi0, t_iter=1e3, dt=2.5e-5, solver_iter=1e4, tol=1e-5, dt_out=1, m=4, epsilon2=np.nan, boundary="periodic", c_relax=2, domain=[1, 0, 1, 0], printres=False, printphi=False, pathname="cd"):
    ...
    if np.isnan(epsilon2):
        ...
        epsilon2 = h2 * m ^ 2 / (2 * np.sqrt(2) * np.atanh(0.9)) ^ 2
    else:
        # Else overwrite m
        m = np.sqrt((epsilon2 * (2 * np.sqrt(2) * np.atanh(0.9)) ^ 2) / h2)
    ...
    if printphi:
        mass_t = np.zeros(n_timesteps + 1, 1)
        E_t = np.zeros(n_timesteps + 1, 1)
        t_out = range(0, (n_timesteps+1)*dt*dt_out, step=dt_out*dt)
        if pathname == "cd":
            pathname = os.pwd()
        ...
        E_t = np.zeros(n_timesteps + 1, 1)
        t_out = range(0, (n_timesteps+1)*dt*dt_out, step=dt_out*dt)
        phi_t = np.zeros(nx, ny, n_timesteps + 1)
        phi_t[:, :, 1] = phi0
    ...
    for it in range(t_iter):
        solver.cahn(phi_old, phi_new, mu, nx, ny, dt, solver_iter, tol, c_relax, xright,
                    xleft, yright, yleft, epsilon2, n_level, boundary, printres=printres)
        phi_old = phi_new.copy()
        if it / t_iter * 100 % 5 == 0:
            print(("%3.0f percent complete", it / t_iter * 100))
        if it % dt_out == 0:
            t_index = np.floor(it / dt_out) + 1
            if printphi:
