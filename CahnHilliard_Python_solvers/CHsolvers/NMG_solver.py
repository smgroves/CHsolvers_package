...
def nonL(c_new, mu_new, nxt, nyt, dt, epsilon2, xright, xleft, yright, yleft, boundary):
    ...
    lap_c = laplace(c_new, nxt, nyt, xright, xleft, yright, yleft, boundary)
    lap_mu = laplace(mu_new, nxt, nyt, xright, xleft, yright, yleft, boundary)
    for i in range(nxt):
        for j in range(nyt):
...
def vcycle(
    ...
    uf_new, wf_new = relax(uf_new, wf_new, su, sw, nxf, nyf, c_relax,
    ...
    if ilevel < n_level:
        ...
        uc_def, wc_def = vcycle(uc_def, wc_def, duc, dwc, nxc, nyc, ilevel + 1,
                                c_relax, xright, xleft, dt, epsilon2, n_level, boundary)
...
def cahn(c_old, c_new, mu, nx, ny, dt, solver_iter, tol, c_relax, xright, xleft, yright, yleft, epsilon2, n_level, boundary, suffix="", printres=true):
    it_mg2 = 0
    resid2 = 1
    ...
    while it_mg2 < solver_iter and resid2 > tol:
        ...
        c_new, mu = vcycle(c_new, mu, sc, smu, nx, ny, 1, c_relax,
                           xright, xleft, yright, yleft, dt, epsilon2, n_level, boundary)
        resid2 = error2(c_old, c_new, mu, nx, ny, dt, xright,
